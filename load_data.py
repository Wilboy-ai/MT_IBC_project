...
#
#     observation_pos_psm1 = _resample(traj.observation['pos_psm1'])
#     observation_pos_psm2 = _resample(traj.observation['pos_psm2'])
#     observation_pos_needle = _resample(traj.observation['pos_needle'])
#     observation_pos_entry = _resample(traj.observation['pos_entry'])
#     observation_image = _resample(traj.observation['image'])
#
...
#         observation={
#             'pos_psm1': observation_pos_psm1,
#             'pos_psm2': observation_pos_psm2,
#             'pos_needle': observation_pos_needle,
#             'pos_entry': observation_pos_entry,
...
def load_tf_record_dataset_sequence(path_to_shards, seq_len):
    ...
    # Get the specification for each dataset file
    specs = []
    ...
    # Define a function that interleave the shards and return sequences of length `seq_len`
    ...
    # Define a function that sets the shape of each observation tensor to [seq_len, ...]
    ...
    # Define the decoder for the dataset files
    decoder = example_encoding.get_example_decoder(specs[0], batched=True, compress_image=True)
    # Load the sharded dataset and interleave the shards
    ...
    # Decode and filter the examples and set the shape of observations
